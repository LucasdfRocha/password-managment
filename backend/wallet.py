...
class WalletManager:
    ...
    def export_wallet(
        ...
        for entry in entries:
            ...
            if isinstance(entry.password, bytes):
                decrypted_password = encryption_manager.decrypt(entry.password)
            elif isinstance(entry.password, str):

                try:
                    password_bytes = base64.b64decode(entry.password)
                    decrypted_password = encryption_manager.decrypt(password_bytes)
                except:

                    password_bytes = entry.password.encode("utf-8")
                    decrypted_password = encryption_manager.decrypt(password_bytes)
            else:
                raise ValueError(f"Formato de senha inválido: {type(entry.password)}")
        ...
        payload = json.dumps(
            wallet_file_data, separators=(",", ":"), sort_keys=True
        ).encode()
    ...
    def import_wallet(
        ...
        wallet_password: str,
        encryption_manager: EncryptionManager,
        storage_manager: Union[object, None] = None,
    ) -> int:
        """
        ...
        Args:
            ...
            encryption_manager: Gerenciador de criptografia principal
            storage_manager: Gerenciador de armazenamento (JsonStorageManager ou compatível com create_entry)
        ...
        salt_b64 = meta.get("wallet_encryption", {}).get(
            "salt"
        ) or wallet_file_data.get("salt")

        if salt_b64:

            wallet_encryption = EncryptionManager(wallet_password, salt_b64)
        else:

            SALT_SIZE = 16
            if len(encrypted_data) < SALT_SIZE:
                raise ValueError("Wallet file corrupted: encrypted data too short")

            salt_from_blob = encrypted_data[:SALT_SIZE]
            wallet_encryption = EncryptionManager(wallet_password, salt_from_blob)
        ...
        if hmac_b64:
            ...
            verify_payload = json.dumps(
                {
                    k: wallet_file_data[k]
                    for k in ("data", "meta")
                    if k in wallet_file_data
                },
                separators=(",", ":"),
                sort_keys=True,
            ).encode()
            ...
            if not hmac.compare_digest(provided, expected):
                raise ValueError(
                    "HMAC mismatch: wallet file may have been tampered with or wrong password"
                )
        ...
        if meta:
            ...
            if wallet_format and wallet_format != WalletManager.WALLET_FORMAT_VERSION:
                print(
                    f"Aviso: formato de wallet diferente ({wallet_format}), import may be partial"
                )

        if not storage_manager:
            raise ValueError("storage_manager é obrigatório para importar wallet")
        ...
        for entry_data in wallet_data["entries"]:
            ...
            entry = PasswordEntry(
                ...
                expiration_date=(
                    datetime.fromisoformat(entry_data["expiration_date"])
                    if entry_data["expiration_date"]
                    else None
                ),
            ...
            storage_manager.create_entry(entry, encrypted_password)
            imported_count += 1
